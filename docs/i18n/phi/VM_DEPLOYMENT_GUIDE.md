# OmniRoute — Gabay sa Deployment sa VM gamit ang Cloudflare
...
- Tandaan ang **pampublikong IP** (hal., `203.0.113.10`)
...
### 2.1 Lumikha ng direktoryo ng pagsasaayos

```bash
mkdir -p /opt/omniroute
```

### 2.2 Lumikha ng file ng mga variable ng kapaligiran
...
> ⚠️ **MAHALAGA**: Bumuo ng mga natatanging lihim na key! Gamitin ang `openssl rand -hex 32` para sa bawat key.

### 2.3 Simulan ang lalagyan
...
### 2.4 I-verify na ito ay tumatakbo
...
Dapat itong magpakita ng: `[DB] SQLite database ready` at `listening on port 20128`.
...
4. Kopyahin ang **Origin Certificate** at ang **Private Key**
...
| ----- | -------- | ---------------------- | ---------- |
| Isang | `llms`   | `203.0.113.10` (VM IP) | ✅ Proxied |
...
- Mga Awtomatikong HTTPS Rewrite: ✅ Naka-on
...
## 5. Mga Operasyon at Pagpapanatili
...
### Manu-manong backup ng database
...
### Limitahan ang nginx sa mga Cloudflare IP
...
### I-block ang direktang access sa Docker port
...
## 7. I-deploy sa Cloudflare Workers (Opsyonal)

Para sa malayuang pag-access sa pamamagitan ng Cloudflare Workers (nang hindi direktang inilalantad ang VM):
